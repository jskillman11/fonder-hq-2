...
//
// isAdmin gates draft visibility explicitly rather than trusting RLS alone —
// change_log's SELECT policy was widened (2026-07-16) to let a client read
// their own program's *published* edit history, but does not filter out
// 'draft' rows, so a client-role caller genuinely gets real unpublished
// drafts back now, not an empty array. Never fetch/forward them for a
// non-admin caller, in the same request AND at the RLS layer (see the
// 2026-08-11 change_log migration) — belt and suspenders, since FonderHQ is
// a Client Component and anything returned here ships in the page payload.
export async function getProgramData(clientId, isAdmin) {
  const supabase = await createClient();
  ...
  // metric. Explicitly filtered to status='published' below, so this one is
  // safe to run for any role — unlike the draft fetch further down.
  ...
  const draftsRaw = isAdmin ? await getPendingDrafts(program.id) : [];
